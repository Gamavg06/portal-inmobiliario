<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full bg-[#080612]">
<head>
    <meta charset="utf-8">
    ...
    <style>
        ...
        .bg-grid-pattern {
            ...
            background-image: 
                linear-gradient(to right, rgba(255, 42, 133, 0.03) 1px, transparent 1px),
                linear-gradient(to bottom, rgba(255, 42, 133, 0.03) 1px, transparent 1px);
        }
        ...
        .shadow-neon-sm {
            box-shadow: 0 0 12px rgba(255, 42, 133, 0.25);
        }
...
<body class="flex flex-col min-h-screen text-slate-200 bg-[#080612] bg-grid-pattern relative selection:bg-[#ff2a85] selection:text-white">
    ...
    <!-- Header Navigation -->
    <header class="sticky top-0 z-50 bg-[#080612]/90 backdrop-blur-xl border-b border-slate-800/80">
                ...
                <div class="flex-shrink-0 flex items-center">
                    <a href="{{ route('properties.index') }}" class="flex items-center space-x-3 group">
                        <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-[#ff2a85] to-purple-600 flex items-center justify-center text-white shadow-neon group-hover:scale-105 transition duration-300">
                            <span class="brand-font font-black text-xl tracking-tight">S</span>
                        </div>
                        <div>
                            <span class="brand-font text-2xl font-extrabold tracking-tight text-white group-hover:text-[#ff2a85] transition duration-200">
                                SGNIA<span class="text-[#ff2a85]">.</span>
                            </span>
                            <span class="block text-[10px] font-bold uppercase tracking-widest text-[#ff2a85]/80 -mt-1">Real Estate</span>
                        </div>
                    </a>
                </div>
                ...
                <div class="flex items-center space-x-4">
                    @auth
                        @if(in_array(Auth::user()->role, ['admin', 'agent']))
                            <a href="{{ route('admin.dashboard') }}" class="hidden sm:inline-flex items-center justify-center px-4 py-2 text-xs font-extrabold text-[#ff2a85] bg-[#ff2a85]/10 border border-[#ff2a85]/40 rounded-xl hover:bg-[#ff2a85] hover:text-white transition duration-200 shadow-neon-sm">
                                👑 Panel Admin
                            </a>
                            <a href="{{ route('admin.properties.create') }}" class="inline-flex items-center justify-center px-4 py-2 text-xs font-extrabold text-white bg-gradient-to-r from-[#ff2a85] to-purple-600 rounded-xl shadow-neon hover:opacity-90 transition duration-200">
                                + Publicar
                            </a>
                        @endif
                        ...
                        <div class="flex items-center space-x-3 border-l border-slate-800/80 pl-4">
                            ...
                            <form action="{{ route('logout') }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="px-4 py-2 text-xs font-bold text-rose-400 hover:text-white hover:bg-rose-600/90 border border-rose-500/30 rounded-xl transition duration-200 shadow-sm">
                                    Salir
                                </button>
                            </form>
                        </div>
                    ...
                    @guest
                        <a href="{{ route('login') }}" class="inline-flex items-center justify-center px-4 py-2.5 text-xs font-extrabold text-slate-300 hover:text-white border border-slate-700/80 hover:border-[#ff2a85] rounded-xl transition duration-200">
                            Iniciar Sesión
                        </a>
                        <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-5 py-2.5 text-xs font-extrabold text-white bg-gradient-to-r from-[#ff2a85] to-purple-600 hover:from-[#e01f73] hover:to-purple-700 rounded-xl shadow-neon transition duration-200">
                            Registrarse
                        </a>
                    @endguest
                </div>
    ...
    <!-- Footer -->
    <footer class="bg-[#05040a] text-slate-400 border-t border-slate-800/80 relative z-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div class="space-y-4 md:col-span-2">
                    <div class="flex items-center space-x-3">
                        ...
                        <span class="brand-font text-xl font-extrabold text-white tracking-tight">SGNIA<span class="text-[#ff2a85]">.</span></span>
                    </div>
